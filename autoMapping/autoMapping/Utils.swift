...
func fetchAPIConversionLocalGlobal(localName: String, nodesList: [(SCNNode, SCNNode)]) async throws -> (HTTPURLResponse?, [String: Any]) {
    ...
    for n in nodesList {
        ...
        _global["position"] = await [
            [n.1.simdWorldTransform.columns.0.x, n.1.simdWorldTransform.columns.0.y, n.1.simdWorldTransform.columns.0.z, n.1.simdWorldTransform.columns.0.w],
    ...
    do {
        ...
        do {
            let d = String(data: bodyres, encoding: .utf8)!.data(using: .utf8)!
    ...
    //let fetchedData = try JSONDecoder().decode(CallForAPI.self, from: try mapResponse(response: (data,response)))
    
    //return fetchedData
    //return ""
}

/*struct CallForAPI: Codable, Hashable {
    
    static func == (lhs: callForAPI, rhs: callForAPI) -> Bool {
